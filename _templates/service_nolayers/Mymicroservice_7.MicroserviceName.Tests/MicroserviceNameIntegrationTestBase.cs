...
public abstract class MicroserviceNameIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(Mymicroservice_7MicroserviceNameModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(Mymicroservice_7MicroserviceNameModule).Assembly.FullName} assembly!");
        }
