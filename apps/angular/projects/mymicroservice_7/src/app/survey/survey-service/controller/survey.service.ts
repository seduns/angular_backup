...
export class SurveyService {
  private apiUrl = 'http://localhost:5081/SurveyService'; // Base API URL
  ...
  // Get survey by ID
  getById(id: string): Observable<surveyItemDto> {
    return this.httpClient.get<surveyItemDto>(`${this.apiUrl}/${id}`).pipe(
      catchError(this.handleError)
    );
  } 
  ...
  update(id: number, survey: surveyItemDto): Observable<surveyItemDto> {
    return this.httpClient.put<surveyItemDto>(`${this.apiUrl}/${id}`, survey).pipe(
      catchError(this.handleError)
    );
  }
