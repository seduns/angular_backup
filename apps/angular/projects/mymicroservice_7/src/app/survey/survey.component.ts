...
import { ProductService } from './survey-service/controller/product.services';
...
@Component({
  ...
  styleUrls: ['./survey.component.scss'] // Fix the property name here
})
export class SurveyComponent implements OnInit {
  ...
  create(form: NgForm, event: Event): void {
    // Mark the form as submitted to show error messages
    this.formSubmitted = true;
  
    // Check if product is selected (prodId should not be 0)
    
  
    // If the form is invalid, mark all fields as touched to trigger validation messages
    if (form.invalid) {
      form.control.markAllAsTouched();  // Trigger form touch to show validation messages
      console.log('Unable to create form');
      alert('Complete all required fields to submit the form.');
      return;
    }
    // Check if rating is selected
   else if (this.newSurvey.prodId === 0) {
    console.error('Product ID is required');
    alert('Please select a product before submitting.');
    return;
  } else if (!this.newSurvey.starRating) {
      console.error('Rating is required');
      alert('Please rate our services before submitting.');
      return;
  } else if (this.newSurvey.comment === '') {
    console.error('Comment required')
    alert('Please choose the available comment');
    return;
  }
