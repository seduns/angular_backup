...
public abstract class AuditLoggingServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(Mymicroservice_7AuditLoggingServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(Mymicroservice_7AuditLoggingServiceModule).Assembly.FullName} assembly!");
        }
