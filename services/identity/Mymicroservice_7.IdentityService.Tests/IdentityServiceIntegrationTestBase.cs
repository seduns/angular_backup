...
public abstract class IdentityServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(Mymicroservice_7IdentityServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(Mymicroservice_7IdentityServiceModule).Assembly.FullName} assembly!");
        }
