...
public abstract class GdprServiceIntegrationTestBase : AbpWebApplicationFactoryIntegratedTest<TestProgram>
{
    ...
    {
        var assemblyDirectoryPath = Path.GetDirectoryName(typeof(Mymicroservice_7GdprServiceModule).Assembly.Location);
        if (assemblyDirectoryPath == null)
        {
            throw new Exception($"Could not find location of {typeof(Mymicroservice_7GdprServiceModule).Assembly.FullName} assembly!");
        }
